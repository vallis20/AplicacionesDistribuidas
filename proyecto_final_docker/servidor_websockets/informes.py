...
def generate_pdf_report(content, filename,Title):
    buffer = BytesIO()
...
def generate_general_report(db, nombreA):
    streaming_query = """
    SELECT
    ...
    generate_pdf_report(content, nombreA,"General Report")
    print("Generando reporte general (estadísticas)")
...
def generate_billing_report(db):
    ...
    for billing_record in billing_stats:
        ...
        content.append(f"   Card: {billing_record.tarjeta}")
        content.append("")  # Add a blank line between entries

    # Generate the PDF report
    generate_pdf_report(content, "Billing_Statistics_Report.pdf",'Billing Statistics Report')
    print("Generando reporte facturación (pagos)")
